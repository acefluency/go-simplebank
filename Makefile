postgres:
	docker run --name postgres12 -p 5433:5432 -e POSTGRES_USER=root -e POSTGRES_PASSWORD=root -d postgres:12-alpine
...
.PHONY: postgres createdb dropdb migrateup migratedown sqlc test run-container